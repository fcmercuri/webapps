...
async function run() {
  if (!PERPLEXITY_API_KEY) {
    console.error('❌ PERPLEXITY_API_KEY secret not set in GitHub');
    process.exit(1);
  }
  ...
  for (const slug of SLUGS_TO_FIX) {
    ...
    try {
      ...
      if (!post.contentHtml) { console.log(`  ⚠️  No contentHtml`); failed++; continue; }
      console.log(`  ✅ Found: ${post.title}`);

      const currentIntro = extractIntro(post.contentHtml);
      if (!currentIntro) { console.log(`  ⚠️  No intro found`); failed++; continue; }

      console.log(`  🤖 Rewriting with Perplexity...`);
      const newIntro = await rewriteIntro(post.title, currentIntro);
      if (!newIntro) { console.log(`  ❌ No response`); failed++; continue; }
